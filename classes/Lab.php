...
class Lab
{
    ...
    public function labRegistration($data)
    {
        ...
        if (empty($file_name) || empty($file_nameimg)) {
           ...
           $msg = "<div class='alert alert-danger'>Passowrd do not match</div>";
            ...
            $mailquery = "SELECT * FROM tbl_lab WHERE email = '$email' LIMIT 1";
            ...
            if ($mailchk != false) {
                ...
                return $msg;
                    ...
                    $query = "INSERT INTO tbl_lab(name,email, phone,state_id,lga_id,address,pname,descr,password,docs,logo, created_at) VALUES('$name', '$email', '$phone', '$state', '$lga', '$address', '$pname', '$desc','$password','$uploaded_image', '$uploaded_imageimg','$this->datetime')";
    ...
    {
        $query = "SELECT * FROM tbl_lab ORDER BY id DESC";
        $result = $this->db->select($query);
        return $result;
    }

    public function laboratoryLogin($phamUser, $phamPass)
    {
        ...
        if (empty($phamUser) || empty($phamPass)) {
            ...
            $query = "SELECT * FROM tbl_lab  WHERE email = '$phamUser' AND password = '$phamPass'";
            $result = $this->db->select($query);
            if ($result != false) {
                $value = $result->fetch_assoc();
                Session::set("lablogin", true);
                Session::set("labId",      $value['id']);
                Session::set("labUser",    $value['name']);
                Session::set("labEmail",   $value['email']);
                Session::set("labPhone",   $value['phone']);
                Session::set("labAddress", $value['address']);
                Session::set("labDesc",    $value['descr']);
                Session::set("labRegDate", $value['created_at']);
                Session::set("labStatus",  $value['status']);
                Session::set("labName",    $value['pname']);
    ...
    public function getLabData($id)
    {
        $query = "SELECT * FROM tbl_lab JOIN states ON tbl_lab.state_id = states.id JOIN lgas ON tbl_lab.lga_id = lgas.id  WHERE tbl_lab.id = '$id'";
        $result = $this->db->select($query);
        return $result;
    }

    public function labUpdate($data, $cmrId)
    {
        $name    	= $this->fm->validation($data['name']);
        $address    = $this->fm->validation($data['address']);
      ...
      public function delLabById($id)
    {
        $query = "DELETE FROM tbl_lab WHERE id = '$id'";
